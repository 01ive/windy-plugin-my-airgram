                    ...
                    <table class="wind-grid">
                        <thead>
                            <tr>
                                ...
                                {#each times as t, j}
                                    <!-- Clic pour mettre à jour l'émagramme -->
                                    <th 
...
<script lang="ts">
    ...
    // Données thermodynamiques (Profils par heure)
    ...
    const fetchWindGrid = async (latitude: number, longitude: number) => {
        ...
        times = []; levels = []; grid = []; hourlyProfiles = [];
        ...
        try {
            ...
            // On récupère le météogramme avec un pas de 3 heures
            ...
            // Extraction des niveaux (basée sur les températures pour assurer les calculs thermo)
            const tempLevels = [];
            ...
            tempLevels.sort((a, b) => b.alt - a.alt); // Plus haut en premier pour la table
            levels = tempLevels.filter(l => l.alt <= 10000);

            // Construction de la Grille (Vent) et des Profils Thermo (Temp/Rosée)
            for (let j = 0; j < times.length; j++) {
                const dataIndex = times[j].index;
                let envData = []; // Pour le graphique (trié de bas en haut)
                
                // On boucle à l'envers pour envData (du sol vers le ciel)
                for (let i = levels.length - 1; i >= 0; i--) {
                    const l = levels[i];
                    // Initialisation grille
                    if (!grid[i]) grid[i] = [];
                    
                    // Remplissage Grille (Vents)
                    ...
                    // Remplissage Profil Thermo (Kelvin -> Celsius)
                    const tK = rawData[`temp-${l.key}`]?.[dataIndex];
                    let rh = rawData[`rh-${l.key}`]?.[dataIndex];
                    
                    if (tK !== undefined) {
                        const tC = tK - 273.15;
                        const tdC = rh !== undefined ? getDewPoint(tC, rh) : tC - 5; // Fallback sécurité
            ...
            if (levels.length > 0 && times.length > 0) {
                ...
                selectHour(0); // Sélectionne la première heure par défaut
            } else {
    ...
    const selectHour = (index: number) => {
        ...
        setTimeout(() => drawSondage(index), 100);
    };

    const drawSondage = (hourIndex: number) => {
        // (window as any).Chart contourne les règles strictes de typage TypeScript Svelte
        const ChartLib = (window as any).Chart;
        if (!ChartLib) {
            // Relance si Chart.js n'a pas encore fini de charger depuis le CDN
            setTimeout(() => drawSondage(hourIndex), 200);
            return;
        }
            ...
            data: {
                datasets: [
                    ...
                    },
                    {
                        ...
                        borderColor: '#f39c12', borderWidth: 1, tension: 0, pointRadius: 0, borderDash: [5, 2]
                    },
                    {
                        label: 'Pt Rosée (°C)',
                        data: dewPoints,
                        borderColor: '#3498db', borderWidth: 2, tension: 0, pointRadius: 0
                    }
                ]
            },
            options: {
    ...
    const onPickerLocation = (location: any) => {
        if (location) {
            ...
            lat = null; lon = null; times = []; levels = []; grid = []; hourlyProfiles = [];
            status = "Cliquez sur la carte.";
        }
    };

    onMount(() => {
        // Injection dynamique sécurisée de Chart.js
...
<style lang="less">
    ...
    tr:last-child td.active-col { border-bottom: 1px solid #333 !important; }
